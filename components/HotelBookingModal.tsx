...
import { CloseIcon, UserIcon, LockIcon, CalendarIcon, BedIcon, CheckCircleIcon } from './icons';
...
const HotelBookingModal: React.FC<HotelBookingModalProps> = ({ activity, travelersCount, onClose, onBookingComplete }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setTimeout(() => {
              onBookingComplete();
              onClose();
            }, 5000); // Close modal after 5 seconds on success
        } catch (err) {
         ...
         return (
             <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                 <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl dark:border dark:border-gray-700 p-8 max-w-md w-full relative">
                      <h2 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100 mb-2">Hotel Already Booked</h2>
                      <p className="text-center text-gray-600 dark:text-gray-400">This hotel stay has already been booked and confirmed.</p>
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl dark:border dark:border-gray-700 p-6 max-w-lg w-full relative max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center mb-4">
                    <BedIcon className="h-8 w-8 text-cyan-600 dark:text-cyan-400 mr-3" />
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Book Your Stay</h2>
                ...
                {confirmation ? (
                     <div className="text-center p-8">
                        ...
                        <h3 className="mt-4 text-2xl font-bold text-green-600 dark:text-green-400">Booking Confirmed!</h3>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-6">You can view your booking details at any time from the 'Stay' tab.</p>
                    </div>
                ) : (
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <h3 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-200">Guest Details</h3>
                            <div className="space-y-3">
                                {guests.map((p, index) => (
                                    <div key={index}>
                                        <label htmlFor={`guest-${index}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Guest {index + 1}</label>
                                        <div className="relative flex-grow">
                                            <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
                                            <input id={`guest-${index}`} type="text" placeholder="Full Name" value={p.name} onChange={(e) => handleGuestChange(index, e.target.value)} required className={inputStyles} />
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div>
                            <h3 className="text-lg font-semibold mb-2 mt-4 text-gray-700 dark:text-gray-200">Payment Details</h3>
                            <div className="space-y-3">
                                <div>
