...
import { CloseIcon, UserIcon, LockIcon, CalendarIcon, PlaneIcon, CheckCircleIcon } from './icons';
         ...
         return (
             <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                 <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl dark:border dark:border-gray-700 p-8 max-w-md w-full relative">
                      <h2 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100 mb-2">Flight Already Booked</h2>
                      <p className="text-center text-gray-600 dark:text-gray-400">This flight has already been booked and confirmed.</p>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl dark:border dark:border-gray-700 p-6 max-w-lg w-full relative max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center mb-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Book Flight</h2>
                ...
                {confirmation ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <h3 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-200">Passenger Details</h3>
                            <div className="space-y-3">
                                {passengers.map((p, index) => (
                                    <div key={index}>
                                        <label htmlFor={`passenger-${index}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Passenger {index + 1}</label>
                                        <div className="relative flex-grow">
                                            <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
                                            <input id={`passenger-${index}`} type="text" placeholder="Full Name" value={p.name} onChange={(e) => handlePassengerChange(index, e.target.value)} required className={inputStyles} />
                                        </div>
                                    </div>
                                ))}
                            </div>
